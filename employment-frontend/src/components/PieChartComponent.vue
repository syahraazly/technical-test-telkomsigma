<template>
  <div class="container mx-auto p-6">
    <div class="bg-white shadow-md rounded-lg p-6">
      <h2 class="text-xl font-semibold text-gray-700 mb-4">Count by Category</h2>
      ...
      <!-- pie Chart -->
      <div>
        <canvas ref="pieChart" class=" max-h-96"></canvas>
      </div>
    </div>
  </div>
</template>
    ...
    async fetchChartData(params = {}) {
      try {
        const response = await axios.get(
          "http://localhost:8000/api/employments/count-by-category",
          { params }
        );

        const data = response.data || [];
        const labels = data.map((item) => item.employment_status || "Unknown");
